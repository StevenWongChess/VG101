...
/* elem: list of elments; 
	card: cardinal of the set; 
	type: data type (CHAR INT or DOUBLE) */
typedef struct universalSet { 
	void *elem; int card; int type; int mem;
} uset;
/* Initialize an empty set of given type and 
allocate the initial memory: INITSETSIZE*type */ 
...
do nothing if the set does not contain this elem;
